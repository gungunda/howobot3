...
async function postJSON(path, body) {
  try {
    ...
    try {
      ...
      log("warn", `${path} bad_json_response`, { status: res.status, bodyPreview: text.slice(0, 200) });
      return { ok: false, error: "bad_json_response", status: res.status, bodyPreview: text.slice(0, 200) };
    }
  } catch (e) {
...
export const SyncService = {
  ...
  async pushSchedule(initData, schedule, clientMeta) {
    const resp = await postJSON("/api/writeSchedule", { initData, schedule, clientMeta });
    if (resp?.ok && resp?.applied) log("log", "pushSchedule applied OK");
    else if (resp?.ok && resp?.applied === false) log("warn", "pushSchedule NOT applied", resp?.reason || "unknown");
    return resp;
  },
  async pushOverride(initData, dateKey, override) {
    const resp = await postJSON("/api/writeOverride", { initData, dateKey, override });
    if (resp?.ok && resp?.applied) log("log", "pushOverride applied OK");
    else if (resp?.ok && resp?.applied === false) log("warn", "pushOverride NOT applied", resp?.reason || "unknown");
    return resp;
  }
};
